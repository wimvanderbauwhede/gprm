...
*/
        ...
        if (operation==M_CoreServices_ALU_not){
            ...
            for(uint argn=0;argn<=n_args-1 ;argn++) {
                ii+=1;
                if (ii>1){
                Word tres_symbol=core_arg(argn);
...
void Base::CoreServices::ls_FPU(Base::CoreServices* ptr) {
    // Set up context
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
     ...
     result_list.push_back(res_symbol);
        ...
        if (operation==M_CoreServices_FPU_not){
            result=1.0-result;
        } else {
            int ii=0;
            uint n_args=nargs();
            for(uint argn=1;argn<=n_args-1 ;argn++) {
                ...
                if (ii>1){
                    ...
                    double tres=getFloat(words);
                     ...
                     switch (operation) {
                     case M_CoreServices_FPU_plus :
                     {
                         result+=tres;
                      break;
                     }
                     case M_CoreServices_FPU_minus :
                     {
                         result-=tres;
                      break;
                     }
                     case M_CoreServices_FPU_times   :
                     {
                         result*=tres;
                      break;
                     }
                     case M_CoreServices_FPU_over :
                     {
                         result=result/tres;
                      break;
                     }
                     case M_CoreServices_FPU_lt :
                     {
                         result=(result<tres)?1:0;
                      break;
                     }
                     case M_CoreServices_FPU_gt :
                     {
                         result=(result>tres)?1:0;
                      break;
                     }
                     case M_CoreServices_FPU_eq :
                     {
                         result=(result==tres)?1:0;
                         break;}
                     default:
...
void Base::CoreServices::ls_BEGIN() {
        ...
        uint n_args=nargs();
        //cout << "======================================******=============" << endl;
        //if(n_args!=0) // Ashkan: Sometimes we produce a begin with no arguments!
        	res=core_arg(n_args-1);
       // else
        //	res=1;
#if VERBOSE==1
 ...
 void Base::CoreServices::ls_IF() {
	 ...
	 uint operation =method();
        if (operation==M_CoreServices_IF_return or operation==M_CoreServices_IF_returntc){
            ...
            unsigned int condval= getUInt(arg(0)) ? 1 : 0;// & 0x1;
            argidx= 2-condval;
        }
...
// otherwise, return it, it will be data, just return it.
	if (is_quoted_ref(argidx)) {
			...
			if (operation==M_CoreServices_IF_returntc
				...
				core_status=CS_done;
				result(ZERO);
				dispatch_reference_temp(argidx);
			}
			else {
...
#if 0
 void Base::CoreServices::ls_S_IF(Base::CoreServices* ptr) {
        // Set up context
CoreServices* servicecore_ptr=(CoreServices*)ptr;
CoreServices& *servicecore_ptr;
System* sba_system_ptr=(System*)(servicecore_ptr->sba_system_ptr);
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
        ...
        if (operation==M_CoreServices_S_IF_return){
 ...
 void Base::CoreServices::ls_Math(Base::CoreServices* ptr) {
        // Set up context
CoreServices* servicecore_ptr=(CoreServices*)ptr;
CoreServices& *servicecore_ptr;
System* sba_system_ptr=(System*)(servicecore_ptr->sba_system_ptr);
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
        ...
        MemAddresses& addresses=addresses();
    Word_List result_list;
        if (operation==M_CoreServices_Math_rand        ){
 ...
 void Base::CoreServices::ls_LET() {
	 ...
	 SBA::Tile& sba_tile=*(sba_system.nodes[this->address]);
        ...
        cout << "LET (" <<service<< ") CORE: " <<current_subtask()<< ": (" <<ppservice<< "<>" <<SC_Base::CoreServices_LET<< "\n";
#endif // VERBOSE
        uint method=method();
        if (method==M_CoreServices_LET_let or method==M_CoreServices_LET_lettc){
#ifdef VERBOSE
            ...
            for(MemAddresses::iterator iter_=addresses.begin();iter_!=addresses.end();iter_++) {
                ...
                if (getQuoted(label)==1){
                    ...
                    Return_to_t return_to=S_CoreServices_LET;
                    Word var_label = setSubtask(label,address);
					var_label = setName(var_label,S_CoreServices_LET);
                ...
                bool use_redir=false;
                    ...
                    if ((last and use_redir)      ){
                        ...
                        Word send_ack_to=setName(label,S_CoreServices_LET);
                        send_ack_to=setSubtask(send_ack_to,address);
                        ack_ok=0;
                        if (method!=M_CoreServices_LET_lettc                       ){
                    ...
                    if ( service!=M_CoreServices_LET_lettc               ){
                        result( reslist );
                    } else {
                        break;
                    }
            ...
            result=var_name;
             ...
             case M_CoreServices_LET_update :
             {
        ...
        default: std::cerr<< "Service "<<service<< " has no implementation\n";exit(0);
            ...
            sba_tile.service_manager.subtask_list.status(current_subtask,STS_cleanup);
              Word_List result_list; result_list.push_back(result); result(result_list);
 } // of not M_CoreServices_LET_let
    } // of ls_LET
#endif // 0
#if 0 // SKIP ls_SEQ

void Base::CoreServices::ls_SEQ(Base::CoreServices* ptr) {
    // Set up context
CoreServices* servicecore_ptr=(CoreServices*)ptr;
CoreServices& *servicecore_ptr;
System* sba_system_ptr=(System*)(servicecore_ptr->sba_system_ptr);
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
...
#ifdef VERBOSE
     int ppservice=(int)service;
    cout << "SEQ (" <<service<< ") CORE: " <<current_subtask<< ": (" <<ppservice<< "<>" <<M_CoreServices_SEQ_seq<< "\n";
    ...
    cout << "SEQ (" <<service<< ") CORE: " <<  "CALLED AS: " <<  sba_tile.service_manager.subtask_list.called_as(current_subtask()) << "\n";
        ...
        for(MemAddresses::iterator iter_=addresses.begin();iter_!=addresses.end();iter_++) {
            ...
            if (getQuoted(label)==1){
                ...
                Return_to_t return_to=S_CoreServices_SEQ;
                Word var_label = setSubtask(label,address);
                var_label = setName(var_label,S_CoreServices_SEQ);
                Word return_as=var_label;
                Word ack_to=0;
                Packet_Type packet_type=P_reference;
                Ctrl_t prio=0;
                ...
                if ((last and use_redir)      ){
                    core_status=CS_managed;
                    Word send_ack_to=setName(label,S_CoreServices_SEQ);
                    send_ack_to=setSubtask(send_ack_to,address);
                    ack_ok=0;
                    if (service!=M_CoreServices_SEQ_seqtc                        ){
                        sba_tile.service_manager.subtask_list.waiting_for_ack(current_subtask,1);
                    }
                    return_as=sba_tile.service_manager.subtask_list.return_as(current_subtask());
                    return_to=sba_tile.service_manager.subtask_list.return_to(current_subtask());
 ...
 void Base::CoreServices::ls_LAMBDA(Base::CoreServices* ptr) {
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
 ...
 void Base::CoreServices::ls_NEW_LAMBDA(Base::CoreServices* ptr) {
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
 ...
 void Base::CoreServices::ls_APPLY(Base::CoreServices* ptr) {
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
        ...
        if (method==M_CoreServices_APPLY_apply){
#ifdef VERBOSE
            ...
            use_unique=true;
              ...
              use_unique=false;
#endif // SEQVM
        } else if (method==M_CoreServices_APPLY_applytc){
#ifdef VERBOSE
 ...
 void Base::CoreServices::ls_NEW_APPLY(Base::CoreServices* ptr) {
            // Set up context
CoreServices* servicecore_ptr=(CoreServices*)ptr;
CoreServices& *servicecore_ptr;
System* sba_system_ptr=(System*)(servicecore_ptr->sba_system_ptr);
...
Tile& sba_tile=*(sba_system.nodes[servicecore_ptr->address]);
    ...
    FILE* fd;
        uint method=method();
        if (method==M_CoreServices_IO_open){
        ...
        } else if (method==M_CoreServices_IO_close){
        ...
        } else if (method==M_CoreServices_IO_readline ){
            if (nargs() == 2){
                Word nbytes=arg(1);
//                Word nbytes=getWord(nbytes_wl); // FIXME: CHECK IF THIS IS OK!
        ...
        } else if (method==M_CoreServices_IO_write                   ){
        ...
        } else if (method==M_CoreServices_IO_eof){
             Word_List res;
            if (feof(fd) ){
                 res.push_back(ONE);
            } else {
            ...
            result( res );
        } else if (method==M_CoreServices_IO_display){
