...
//#ifdef VERBOSE
//    cout << "Gateway::Mapping the Chip_Array global structure\n";
//#endif // VERBOSE
 ...
 void Gateway::parse_task_description()  {
    GatewayTile& gw_tile=*((GatewayTile*)sba_gwtile_ptr);
#if VERBOSE==1
    cout << "Gateway: parse_task_description("<<task<<")"<<endl;
#endif // VERBOSE
                    core_status = CS_busy;
                    if (!gw_tile.reuse) {
...
//                        cout << "NPACKETS: "<<task_description_packet_list.size()<<"\n";
                    for(Packet_List::iterator iter_=task_description_packet_list.begin();iter_!=task_description_packet_list.end();iter_++) {
                    	...
                    	if ((getType(getHeader(task_description_packet))) == P_reference) {
                    		root_ref_packet=task_description_packet;
#if VERBOSE==1
                        cout << "GATEWAY SENDS ROOT REF PACKET:"<<endl;
...
#if VERBOSE==1
                        cout << "GATEWAY SENDS ROOT REF PACKET:"<<endl;
                        cout << ppPacket(root_ref_packet)<<endl;
#endif // VERBOSE
                        transmit_packet(root_ref_packet);
                    }
	gw_tile.init = false;
...
#endif
#ifdef USE_TILERA
                     sba_system.result=NULL; //TODO: Ashkan: not sure if NULL is a good replacement!
#else
...
//                        }
    ...
    unmap_shared();
#ifdef VERBOSE
    cout << "Gateway::Unmapping the Chip_Array global structure\n";
#endif // VERBOSE
#endif
