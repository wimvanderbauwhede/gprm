...
def parseCommandline():
    ...
    try:
        options, patterns = getopt.getopt( sys.argv[1:], 'o:r:',
                                           ['version', 'output=', 'runner=', 'gui=','systemc',
...
def setOptions( options ):
    ...
    global outputFileName, templateFileName, runner, gui, haveStandardLibrary, factor, longlong, systemC
...
def writeOutput(outputFileName,runner,gui):
    '''Create output file'''
    if templateFileName:
        writeTemplateOutput(outputFileName,runner)
    else:
        writeSimpleOutput(outputFileName,runner,gui)

def writeSimpleOutput(outputFileName,runner,gui):
    '''Create output not based on template'''
    output = startOutputFile(outputFileName)
    writePreamble( output,runner,gui )
    writeMain( output,runner, gui )
    writeWorld( output, runner,gui )
    output.close()
...
def writeTemplateOutput(runner):
    ...
    while 1:
        ...
        if include_re.search( line ):
            writePreamble( output,runner )
            output.write( line )
        elif preamble_re.search( line ):
            writePreamble( output,runner )
        elif world_re.search( line ):
            writeWorld( output, runner )
        else:
            output.write( line )
    template.close()
    output.close()

def startOutputFile(outputFileName):
...
wrotePreamble = 0
def writePreamble( output, runner,gui ):
...
def writeMain( output,runner,gui ):
...
wroteWorld = 0
def writeWorld( output, runner,gui ):
...
def createRunner(files,outputFileName,runner,gui,sysc):
    global systemC
    systemC=sysc
    scanInputFiles( files )
    writeSimpleOutput(outputFileName,runner,gui)
